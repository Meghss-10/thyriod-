...
def graph():
    df = pd.DataFrame([
        ...
        ['Random Forest', 'Precision', precision[2]],
        ['Random Forest', 'Recall', recall[2]],
        ['Random Forest', 'F1 Score', fscore[2]],
        ['Random Forest', 'Accuracy', accuracy[2]]
    ], columns=['Algorithm', 'Metric', 'Value'])

    pivot_df = df.pivot_table(index='Metric', columns='Algorithm', values='Value')
